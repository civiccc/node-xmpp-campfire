  ...
  handle_enter: function(stanza) {
    ...
    Step(
      function auth() {
        ...
        if (i > -1) {
          options.account = self.jid.user.substr(0, i);
          room_name = self.jid.user.substr(i + 1);
        } else {
          throw 302;
        }
      ...
      function find_room(err, rooms) {
        ...
        rooms.some(function(room) {
          if (room.id == room_name || room.name == room_name) {
            found_room = room;
            return true;
          }
          return false;
      ...
      if (!err) {
        try {
          ...
          self.campfire_roster.forEach(function(user_id) {
            if (roster.indexOf(user_id) < 0) {
              self.send_stanza(
                self.campfire_users[user_id], build_xmpp_presence(
                  {type: 'unavailable'}));
            }
          });
