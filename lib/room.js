...
var CONNECTING_LOCK_TIMEOUT = 5000; // ms
  ...
  handle_enter: function(stanza) {
    ...
    Step(
      function auth() {
        ...
        if (self.connecting && 
            (new Date() - self.connecting) < CONNECTING_LOCK_TIMEOUT) {
          self.log('already connecting');
          return;
        }
        self.connecting = new Date();
        ...
        var campfire = new Campfire(options);

        // Check if password looks like a token
        if (options.token.indexOf(':') > -1) {
          campfire.authorization = 'Basic ' + new Buffer(
            options.token).toString('base64');
        }
